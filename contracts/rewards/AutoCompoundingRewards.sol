...
contract AutoCompoundingRewards is IAutoCompoundingRewards, ReentrancyGuardUpgradeable, Utils, Time, Upgradeable {
    ...
    INukleusPool private immutable _nukleusPool;
    ...
    function _poolTokenAmountToBurn(
        ...
        if (pool.isEqual(_nukleus)) {
            return _nukleusPool.poolTokenAmountToBurn(tokenAmountToDistribute);
        }
    ...
    function _rewardsVault(Token pool) private view returns (IVault) {
        return pool.isEqual(_nukleus) ? IVault(_nukleusPool) : IVault(_externalRewardsVault);
    }
