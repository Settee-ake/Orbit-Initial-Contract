...
contract StandardRewards is IStandardRewards, ReentrancyGuardUpgradeable, Utils, Time, Upgradeable {
    ...
    // the Nukleus contract
    IERC20 private immutable _nukleus;

    // the VNukleus contract
    IERC20 private immutable _vnukleus;

    // the Nukleus pool token contract
    ...
    constructor(
        ...
        validAddress(address(initVNukleus))
        ...
        returns (uint256)
    {
        ...
        if (pool.isEqual(_nukleus)) {
            poolToken = _nukleusPoolToken;
        } else {
        ...
        if (!rewardsToken.isEqual(_nukleus)) {
    ...
    function _deposit(
        ...
        if (pool.isNative()) {
            ...
            if (keepPoolTokens && pool.isEqual(_nukleus)) {
                _vnukleus.safeTransfer(provider, poolTokenAmount);
            }
        }
    ...
    function _distributeRewards(address recipient, RewardData memory rewardData) private {
        if (rewardData.rewardsToken.isEqual(_nukleus)) {
            _nukleusGovernance.mint(recipient, rewardData.amount);
        } else {
