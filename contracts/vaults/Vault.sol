...
import { ITokenGovernance } from "@nukleus/token-governance/contracts/ITokenGovernance.sol";
...
abstract contract Vault is IVault, Upgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable, Utils {
    ...
    IERC20 internal immutable _nukleus;

    // the address of the Nukleus token governance
    ITokenGovernance internal immutable _nukleusGovernance;

    // the address of the VNukleus token
    IERC20 internal immutable _vnukleus;

    // the address of the VNukleus token governance
    ITokenGovernance internal immutable _vnukleusGovernance;
     ...
     */
    constructor(ITokenGovernance initNukleusGovernance, ITokenGovernance initVNukleusGovernance)
        validAddress(address(initNukleusGovernance))
        validAddress(address(initVNukleusGovernance))
    {
        ...
        whenAuthorized(msg.sender, token, payable(address(0)), amount)
    {
        ...
        // allow vaults to burn Nukleus and VNukleus via their respective token governance modules
        if (token.isEqual(_nukleus)) {
            _nukleusGovernance.burn(amount);
        } else if (token.isEqual(_vnukleus)) {
            _vnukleusGovernance.burn(amount);
        } else {
