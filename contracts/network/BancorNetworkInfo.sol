...
import { INukleusPool } from "../pools/interfaces/INukleusPool.sol";
...
import { INukleusNetworkInfo } from "./interfaces/INukleusNetworkInfo.sol";
import { INukleusNetwork } from "./interfaces/INukleusNetwork.sol";
...
/**
 * @dev Nukleus Network Information contract
 */
contract NukleusNetworkInfo is INukleusNetworkInfo, Upgradeable, Utils {
    using TokenLibrary for Token;

    // the address of the network
    IKleoniaNetwork private immutable _network;

    // the address of the Nukleus token
    IERC20 private immutable _nukleus;

    // the address of the Nekleus token governance
    ITokenGovernance private immutable _nukleusGovernance;

    // the address of the VNukleus token
    IERC20 private immutable _vnukleus;

    // the address of the VNukleus token governance
    ITokenGovernance private immutable _vnukleusGovernance;
    ...
    IBNTPool private immutable _nukleusPool;

    // the Nukleus pool token
    IPoolToken private immutable _nukleusPoolToken;
    ...
    constructor(
        ...
        IBNTPool initNukleusPool,
        IPendingWithdrawals initPendingWithdrawals,
        IPoolMigrator initPoolMigrator
    ) {
        _validAddress(address(initNetwork));
        _validAddress(address(initNukleusGovernance));
        _validAddress(address(initVNukleusGovernance));
        ...
        _validAddress(address(initNukleusPool));
        _validAddress(address(initPendingWithdrawals));
        _validAddress(address(initPoolMigrator));

        _network = initNetwork;
        _nukleusGovernance = initNukleusGovernance;
        _nukleus = initNukleusGovernance.token();
        _vnukleusGovernance = initVNukleusGovernance;
        _vnukleus = initVNukleusGovernance.token();
        _networkSettings = initNetworkSettings;
        _masterVault = initMasterVault;
        _externalProtectionVault = initExternalProtectionVault;
        _externalRewardsVault = initExternalRewardsVault;
        _nukleusPool = initNukleusPool;
        _nukleusPoolToken = initNukleusPool.poolToken();
