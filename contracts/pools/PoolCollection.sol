...
import { Sint256, MathExtend } from "../utility/MathExtend.sol";
...
import { INukleusPool } from "./interfaces/INukleusPool.sol";
...
struct InternalWithdrawalAmounts {
    ...
    uint256 nukleusToMintForProvider; // Nukleus amount to mint directly for the provider
    ...
    Sint256 nukleusTradingLiquidityDelta; // Nukleus amount to add to the trading liquidity and to the master vault
    Sint256 nukleusProtocolHoldingsDelta; // Nukleus amount add to the protocol equity
    ...
    uint256 newNukleusTradingLiquidity; // new Nukleus trading liquidity
}
...
contract PoolCollection is IPoolCollection, Owned, BlockNumber, Utils {
    ...
    // the address of the Nukleus token
    IERC20 private immutable _nukleus;
    ...
    // the Nukleus pool contract
    INukleusPool internal immutable _nukleusPool;
     ...
     */
    constructor(
        INukleusNetwork initNetwork,
        IERC20 initNukleus,
        INetworkSettings initNetworkSettings,
        IMasterVault initMasterVault,
        INukleusPool initNukleusPool,
        IExternalProtectionVault initExternalProtectionVault,
        IPoolTokenFactory initPoolTokenFactory,
        IPoolMigrator initPoolMigrator
    )
        validAddress(address(initNetwork))
        validAddress(address(initNukleus))
        validAddress(address(initNetworkSettings))
        validAddress(address(initMasterVault))
        validAddress(address(initNukleusPool))
        ...
        validAddress(address(initPoolMigrator))
    {
        ...
        _nukleus = initNukleus;
        _networkSettings = initNetworkSettings;
        _masterVault = initMasterVault;
        _nukleusPool = initNukleusPool;
    ...
    function createPool(Token token) external only(address(_network)) {
        ...
        Pool memory newPool = Pool({
            ...
            depositLimit: 0,
            liquidity: PoolLiquidity({ nukleusTradingLiquidity: 0, baseTokenTradingLiquidity: 0, stakedBalance: 0 })
        });
     ...
     * tokens. For example:
     *
     * - if the price of one (10**18 wei) Nukleus is $X and the price of one (10**18 wei) TKN is $Y, then the virtual balances
     *   should represent a ratio of X to Y
     * - if the price of one (10**18 wei) Nukleus is $X and the price of one (10**6 wei) USDC is $Y, then the virtual balances
    ...
    function enableTrading(
        Token pool,
        uint256 nukleusVirtualBalance,
        uint256 baseTokenVirtualBalance
    ) external onlyOwner {
        Fraction memory fundingRate = Fraction({ n: nukleusVirtualBalance, d: baseTokenVirtualBalance });
        ...
        // verify that the Nukleus trading liquidity is equal or greater than the minimum liquidity for trading
        if (data.liquidity.nukleusTradingLiquidity < minLiquidityForTrading) {
        ...
        returns (WithdrawalAmounts memory)
    {
        ...
        return
            WithdrawalAmounts({
                ...
                nukleusAmount: amounts.nukleusToMintForProvider
            });
    }
    ...
    function _poolWithdrawalAmounts(
        ...
        PoolCollectionWithdrawal.Output memory output = PoolCollectionWithdrawal.calculateWithdrawalAmounts(
            data.liquidity.nukleusTradingLiquidity,
            data.liquidity.baseTokenTradingLiquidity,
        ...
        return
            InternalWithdrawalAmounts({
                ...
                nukleusToMintForProvider: output.t,
                baseTokensToTransferFromEPV: output.u,
                baseTokensTradingLiquidityDelta: output.r,
                nukleusTradingLiquidityDelta: output.p,
                nukleusProtocolHoldingsDelta: output.q,
                ...
                newBaseTokenTradingLiquidity: output.r.isNeg
                    ...
                    : data.liquidity.baseTokenTradingLiquidity + output.r.value,
                newNukleusTradingLiquidity: output.p.isNeg
                    ? data.liquidity.nukleusTradingLiquidity - output.p.value
                    : data.liquidity.nukleusTradingLiquidity + output.p.value
            });
    }
     ...
     * - update the pool's Nukleus trading liquidity
     * - update the pool's trading liquidity product
     * - emit an event if the pool's Nukleus trading liquidity has crossed the minimum threshold
    ...
    function _executeWithdrawal(
        ...
        liquidity.nukleusTradingLiquidity = SafeCast.toUint128(amounts.newNukleusTradingLiquidity);

        if (amounts.nukleusProtocolHoldingsDelta.value > 0) {
            assert(amounts.nukleusProtocolHoldingsDelta.isNeg); // currently no support for requesting funding here

            _nukleusPool.renounceFunding(contextId, pool, amounts.nukleusProtocolHoldingsDelta.value);
        } else if (amounts.nukleusTradingLiquidityDelta.value > 0) {
            if (amounts.nukleusTradingLiquidityDelta.isNeg) {
                _nukleusPool.burnFromVault(amounts.nukleusTradingLiquidityDelta.value);
            } else {
                _nukleusPool.mint(address(_masterVault), amounts.nukleusTradingLiquidityDelta.value);
            }
        }

        // if the provider should receive some Nukleus - ask the Nukleus pool to mint Nukleus to the provider
        if (amounts.nukleusToMintForProvider > 0) {
            _nukleusPool.mint(address(provider), amounts.nukleusToMintForProvider);
        }
        ...
        // if the new Nukleus trading liquidity is below the minimum liquidity for trading - reset the liquidity
        if (amounts.newNukleusTradingLiquidity < _networkSettings.minLiquidityForTrading()) {
            _resetTradingLiquidity(
                contextId,
                pool,
                data,
                amounts.newNukleusTradingLiquidity,
                TRADING_STATUS_UPDATE_MIN_LIQUIDITY
            );
        }
    ...
    /**
     * @dev returns the target Nukleus trading liquidity, and whether or not it needs to be updated
     */
    function _calcTargetNukleusTradingLiquidity(
        ...
        // calculate the target Nukleus trading liquidity based on the smaller between the following:
        // - Nukleus liquidity required to match previously deposited based token liquidity
        // - maximum available Nukleus trading liquidity (current amount + available funding)
        uint256 targetNukleusTradingLiquidity = Math.min(
            MathEx.mulDivF(tokenReserveAmount, fundingRate.n, fundingRate.d),
            liquidity.nukleusTradingLiquidity + availableFunding
        );

        // ensure that the target is above the minimum liquidity for trading
        if (targetNukleusTradingLiquidity < minLiquidityForTrading) {
            return TradingLiquidityAction({ update: true, newAmount: 0 });
        }

        // calculate the new Nukleus trading liquidity and cap it by the growth factor
        if (liquidity.nukleusTradingLiquidity == 0) {
            // if the current Nukleus trading liquidity is 0, set it to the minimum liquidity for trading (with an
            // additional buffer so that initial trades will be less likely to trigger disabling of trading)
            uint256 newTargetNukleusTradingLiquidity = minLiquidityForTrading * BOOTSTRAPPING_LIQUIDITY_BUFFER_FACTOR;

            // ensure that we're not allocating more than the previously established limits
            if (newTargetNukleusTradingLiquidity > targetNukleusTradingLiquidity) {
                return TradingLiquidityAction({ update: false, newAmount: 0 });
            }

            targetNukleusTradingLiquidity = newTargetNukleusTradingLiquidity;
        } else if (targetNukleusTradingLiquidity >= liquidity.nukleusTradingLiquidity) {
            ...
            targetNukleusTradingLiquidity = Math.min(
                targetNukleusTradingLiquidity,
                liquidity.nukleusTradingLiquidity * LIQUIDITY_GROWTH_FACTOR
            );
        }

        return TradingLiquidityAction({ update: true, newAmount: targetNukleusTradingLiquidity });
    }
    ...
    function _updateTradingLiquidity(
        ...
        TradingLiquidityAction memory action = _calcTargetNukleusTradingLiquidity(
            tokenReserveAmount,
            _nukleusPool.availableFunding(pool),
            liquidity,
            fundingRate,
        ...
        // update funding from the Nukleus pool
        if (action.newAmount > liquidity.nukleusTradingLiquidity) {
            _nukleusPool.requestFunding(contextId, pool, action.newAmount - liquidity.nukleusTradingLiquidity);
        } else if (action.newAmount < liquidity.nukleusTradingLiquidity) {
            _nukleusPool.renounceFunding(contextId, pool, liquidity.nukleusTradingLiquidity - action.newAmount);
        }

        // calculate the base token trading liquidity based on the new Nukleus trading liquidity and the effective
        // funding rate (please note that the effective funding rate is always the rate between Nukleus and the base token)
        ...
        PoolLiquidity memory newLiquidity = PoolLiquidity({
            nukleusTradingLiquidity: SafeCast.toUint128(action.newAmount),
    ...
    function _dispatchTradingLiquidityEvents(
        ...
        if (newLiquidity.nukleusTradingLiquidity != prevLiquidity.nukleusTradingLiquidity) {
            emit TradingLiquidityUpdated({
                contextId: contextId,
                pool: pool,
                token: Token(address(_nukleus)),
                prevLiquidity: prevLiquidity.nukleusTradingLiquidity,
                newLiquidity: newLiquidity.nukleusTradingLiquidity
            });
        }
    ...
    /**
     * @dev resets trading liquidity and renounces any remaining Nukleus funding
     */
    function _resetTradingLiquidity(
        ...
        _resetTradingLiquidity(contextId, pool, data, data.liquidity.nukleusTradingLiquidity, reason);
    }
    ...
    function _resetTradingLiquidity(
        ...
        Token pool,
        Pool storage data,
        uint256 currentNukleusTradingLiquidity,
        uint8 reason
    ) private {
        // reset the network and base token trading liquidities
        data.liquidity.nukleusTradingLiquidity = 0;
        ...
        if (currentNukleusTradingLiquidity > 0) {
            _nukleusPool.renounceFunding(contextId, pool, currentNukleusTradingLiquidity);
        }
    }
    ...
    function _initTrade(
        ...
        // ensure that Nukleus is either the source or the target token
        bool isSourceNukleus = sourceToken.isEqual(_nukleus);
        bool isTargetNukleus = targetToken.isEqual(_nukleus);

        if (isSourceNukleus && !isTargetNukleus) {
            result.isSourceNukleus = true;
            result.pool = targetToken;
        } else if (!isSourceNukleus && isTargetNukleus) {
            result.isSourceNukleus = false;
            result.pool = sourceToken;
        } else {
            // Nukleus isn't one of the tokens or is both of them
            revert DoesNotExist();
        }
        ...
        if (result.isSourceNukleus) {
            result.sourceBalance = liquidity.nukleusTradingLiquidity;
            result.targetBalance = liquidity.baseTokenTradingLiquidity;
        } else {
            result.sourceBalance = liquidity.baseTokenTradingLiquidity;
            result.targetBalance = liquidity.nukleusTradingLiquidity;
        }
    ...
    function _processNetworkFee(TradeIntermediateResult memory result) private view {
        ...
        // trade the network fee (taken from the base token) to Nukleus
        ...
        // since we have received the network fee in base tokens and have traded them for Nukleus (so that the network fee
        // is always kept in Nukleus), we'd need to adapt the trading liquidity and the staked balance accordingly
        result.targetBalance += targetNetworkFeeAmount;
    ...
    function _performTrade(TradeIntermediateResult memory result) private {
        ...
        _updateAverageRate(
            data,
            Fraction({ n: prevLiquidity.nukleusTradingLiquidity, d: prevLiquidity.baseTokenTradingLiquidity })
        );
        ...
        // trading liquidity is assumed to never exceed 128 bits (the cast below will revert otherwise)
        PoolLiquidity memory newLiquidity = PoolLiquidity({
            nukleusTradingLiquidity: SafeCast.toUint128(result.isSourceNukleus ? result.sourceBalance : result.targetBalance),
            baseTokenTradingLiquidity: SafeCast.toUint128(
                result.isSourceNukleus ? result.targetBalance : result.sourceBalance
            ),
        ...
        returns (PoolRateState)
    {
        Fraction memory spotRate = Fraction({
            n: liquidity.nukleusTradingLiquidity,
