...
import { Sint256, Uint512, MathExtend } from "../utility/MathExtend.sol";
 ...
 * `p` - Nukleus amount to add to the trading liquidity and to the master vault
 * `q` - Nukleus amount to add to the protocol equity
 * `r` - base token amount to add to the trading liquidity
 * `s` - base token amount to transfer from the master vault to the provider
 * `t` - Nukleus amount to mint directly for the provider
...
library PoolCollectionWithdrawal {
    ...
    function calculateWithdrawalAmounts(
        ...
        if ((e * (M - n)) / M > b + c) {
            ...
            uint256 f = MathExtend.subMax0(b + c, e);
    ...
    function affordableDeficit(
        ...
        // given the restrictions above, everything below can be declared `unchecked`
        Uint512 memory lhs = MathExtend.mul512(b * e, f * m + e * n);
        Uint512 memory rhs = MathExtend.mul512(f * x, g * (M - m));
        return MathExtend.gt512(lhs, rhs);
    }
    ...
    function affordableSurplus(
        ...
        // given the restrictions above, everything below can be declared `unchecked`
        Uint512 memory lhs = MathExtend.mul512(b * e, (f * m + e * n) * M);
        Uint512 memory rhs = MathExtend.mul512(f * x, (f * M + e * n) * (M - m));
        return MathExtend.gt512(lhs, rhs); // `x < e*c/(b+c)` --> `f*x < e*c*(b+c-e)/(b+c) <= e*c <= 2**256-1`
    }
    ...
    function arbitrageDeficit(
        ...
        output.p = MathExtend.mulDivF(a * x, i, j).toPos256();
        output.r = MathExtend.mulDivF(x, f, e).toNeg256();
        output.s = y;
    }
    ...
    function arbitrageSurplus(
        ...
        output.p = MathExtend.mulDivF(a * x, i, j).toNeg256();
        output.r = MathExtend.mulDivF(x, i, e * M).toPos256();
        output.s = y;
    }
    ...
    function defaultDeficit(
        ...
        // given the restrictions above, everything below can be declared `unchecked`
        uint256 z = MathExtend.subMax0(y * b, c * (e - y));
        output.p = MathExtend.mulDivF(a, z, b * e).toNeg256();
        output.q = output.p;
        output.r = (z / e).toNeg256();
        output.s = MathExtend.mulDivF(y, b + c, e);
    }
    ...
    function defaultSurplus(
        ...
        // given the restrictions above, everything below can be declared `unchecked`
        uint256 z = MathExtend.subMax0(y, c);
        output.p = MathExtend.mulDivF(a, z, b).toNeg256();
        output.q = output.p;
        output.r = z.toNeg256();
        output.s = y;
    }
    ...
    function externalProtection(
        ...
        uint256 we = w * e;
        if (yg > we) {
            t = a > 0 ? MathExtend.mulDivF(a, yg - we, b * e) : 0;
            u = w;
        } else {
            t = 0;
            u = yg / e;
    ...
    function mulAddMulDivF(
        ...
        return a * b + MathExtend.mulDivF(x, y, z);
    }
